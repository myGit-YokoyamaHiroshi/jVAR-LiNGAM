...
def plot_phase_interaction(omega, K1, K2, Nosc):
    P = K1.shape[2]
    ...
    for ref in range(Nosc):
        ...
        for osc in range(Nosc):
            if osc != ref:
                ...
                plt.plot(phi_delta_plot, prc_model, c='c', linewidth = 2, zorder=2, label='true')
                
                plt.xlabel('$\\theta_{%d} - \\theta_{%d} $'%(osc, ref))
                plt.ylabel('$d \\theta_{%d} / dt $'%(ref))
                plt.xticks([0, np.pi, 2 * np.pi], ['$0$', '$\\pi$', '$2 \\pi$'])
                
                ylims   = np.array([omega[ref]- 1, omega[ref] + 1]) 
                plt.ylim(ylims)
            elif osc==ref:
                ax = plt.subplot(gs[ref, osc])
                plt.text(0, 0, 'to $\\theta_{%d}$'%(ref), fontsize=20)
                plt.xlim(-.1, 1)
                plt.ylim(-.1, .5)
                ax.set_axis_off()
...
omega  = 2*np.pi* np.array([10.505, 10.69, 10.839])
...
phi_st, phi_plot, prob_phi = solve_steady_state(omega, K1, K2, h,
                                                noise_scale=1,
                                                Ndelta=36)
J = get_jacobian_matrix(phi_st, K1, K2, omega)
...
check_estimated_equilibrium_points(theta, phi_st, omega, h, 
                                   '', phi_plot, prob_phi)
plt.show()
#%%
############################################################
##### Bayesian regression based estimation ##################
############################################################
P           = 2
Tstep       = 1
noise_param = np.sqrt(h)#1E-1#1E-2 # covariance of process noise
prec_param  = 1/noise_param # precision parameter, cov(process noise) = 1/prec_param
model       = est_model(theta, Tstep, Nosc, P, h, prec_param) 
...
########################################################
fig_dir = current_path + '/figures/dyVARLiNGAM_Nosc3_with_est/'
...
plot_phase_interaction(omega, K1, K2, Nosc)
plt.savefig(fig_dir + 'phase_interaction.png', bbox_inches="tight")
plt.savefig(fig_dir + 'phase_interaction.svg', bbox_inches="tight")
plt.show()

##### visualize estimated phase interaction function

_, pos = vis_directed_graph(Kest.T, vmin, vmax, seed, pos=pos)
plt.title('Network structure')
...
plot_phase_interaction(omega_est, K1est, K2est, Nosc)
...
theta_est, _, time = solve_oscillator(x0, h, 
                                      K1est, K2est, omega_est, 
                                      Nt, Nosc)

x  = np.unwrap(theta_est[:-1,:].T).T
y  = np.unwrap(theta_est[1:,:].T).T

#%% [3] Step.3 : Calculate Jacobian matrix 
M   = expm(J*h)
#%% [4] Step.4 : Estimate instantaneous effects B0 with LiNGAM
...
if len(pos)==0:
    _, pos = vis_directed_graph(Kest.T, vmin, vmax, 
                                seed, cbar=False, ax=axes[0,0])
else:
    vis_directed_graph(Kest.T, vmin, vmax, 
...
im = axes[1,0].imshow(Kest, cmap='RdBu_r', interpolation='none',
                    vmin=vmin, vmax=vmax)
